...
- `GemTok-TikFinity-Kopru.bat`, `tools/gemtok-tikfinity-bridge.mjs`
- `sıra/` (içinde `gemtok-license-registry.json` olmalı)
...
## 6) TikFinity (Hostinger HTTPS sitesi)

Yayıncının bilgisayarında TikFinity çalışır; site **HTTPS** olduğu için tarayıcı doğrudan `ws://127.0.0.1:21213` adresine bağlanamaz. GemTok **köprü** ile bu engeli aşar:

1. **TikFinity** masaüstü uygulamasını açın (canlı yayın bağlı).
2. Proje kökünde **`GemTok-TikFinity-Kopru.bat`** dosyasını çalıştırın (Node.js gerekir; ilk seferde `ws` paketini kurar).
3. Hostinger sitesinde **Oyun Merkezi** → lisans anahtarını uygulayın → oyunu açın (**aynı tarayıcı sekmesi**).
4. Chrome/Edge **«Yerel ağ cihazlarına bağlanmaya izin ver»** uyarısında **İzin ver** deyin.
5. Üst menüde **TikTok Live · Bağlı** yeşile dönmeli.

Köprü adresi: `ws://127.0.0.1:29213` → TikFinity `21213`. Pakette `tools/gemtok-tikfinity-bridge.mjs` ve `GemTok-TikFinity-Kopru.bat` bulunur.

**Lisans kaydı:** `sıra/gemtok-license-registry.json` dosyasını Hostinger **`sıra/`** klasörüne yükleyin (Admin panelden export veya hazır 200 anahtar dosyası).
